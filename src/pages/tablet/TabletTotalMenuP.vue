      ...
      <div class="gnb_menu_tablet--body">
        ...
        >
          ...
          >
            ...
            <ul class="gnb_menu_tablet--ul-depth2">
              <li
...
<script setup>
import { chain, find, get, sortBy } from 'lodash-es';
...
const bookmarks = computed(() => ({
  ...
  menus: getters['meta/getBookmarks'].filter((menu) => menu.folderYn === 'N').map((bookmark) => (
    {
      menuUid: bookmark.menuUid, menuName: bookmark.bookmarkName, pageId: bookmark.pageId,
    })),
}));
...
async function updateBookmark(isCreate, menu) {
  if (isAuthenticated) {
    ...
    groupedDepth3Menus.value[0] = bookmarks.value;
  }
}
